...
test("dashboard exposes monitoring, export, docs, alerts, and API key quota features", () => {
  ...
  [
    ...
    assert.ok(server.includes(fragment), `${fragment} is missing`),
  );
});
...
test("OpenAPI contract is published as valid JSON", () => {
  const openapi = JSON.parse(read("public/openapi.json"));
  assert.equal(openapi.openapi, "3.0.3");
  assert.ok(openapi.paths["/integrator/{service}/{path}"]);
  assert.ok(openapi.paths["/api/tokens/revoke"]);
});
